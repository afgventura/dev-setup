...
if [ "$what" = all ] || [ "$what" = terminal ]; then
  ...
  for f in ui.conf ghostty-ui.sh sidebar.sh sidebar-list.sh sidebar-refresh.sh sidebar-click.sh sidebar-pos.sh sidebar-nav.sh open-url.sh copy-release.sh agent-notify.sh selftest.sh clicktest.py; do
