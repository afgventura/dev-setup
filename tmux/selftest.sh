...
sleep 0.8; ROW=$($T -L ui capture-pane -p -t ui:.0 | grep -n "▶" | cut -d: -f1); EXP=$(( 2 * $(row "$($T display -t main -p '#{window_id}')") + 3 )); [ "$ROW" = "$EXP" ] && pass "▶ tracks active tab" || fail "▶ on line $ROW, expected $EXP"
R=$(row "$NEWID"); $T -L ui run-shell "~/.config/tmux/sidebar-click.sh $((2*R+2))"; sleep 0.8; A=$($T display -t main -p '#I'); [ "$A" = "$NEW" ] && pass "click row $R → tab $NEW" || fail "click row $R → $A"
