...
tabs_listed(){ $T -L ui capture-pane -p -t ui:.0 | grep -cE "^  [▶• ] "; }                  # rows that are tabs/sessions
...
sleep 1; SB=$(tabs_listed); EXP=$((N1 + $($T list-sessions | grep -vc "^main:"))); [ "$SB" -eq "$EXP" ] && pass "sidebar lists $SB rows (tabs + sessions)" || fail "sidebar lists $SB rows, expected $EXP"
...
sleep 1; SB=$(tabs_listed); EXP=$((N2 + $($T list-sessions | grep -vc "^main:"))); [ "$SB" -eq "$EXP" ] && pass "sidebar updates after close" || fail "sidebar shows $SB after close, expected $EXP"
